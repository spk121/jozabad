...
do_y_call_accepted(Channel c) {
    ...
    if (!validate(c.packet_size_index)) {
        c.do_clear(&connection_store, diagnostic);
...
channel_dispatch(Channel c, const char *x_dname, const char *y_dname, const msg_t *msg) {
    ...
    switch (a) {
        ...
        case a_discard:
            break;
        case a_reset:
            c.do_reset(&connection_store, diagnostic);
            break;
        case a_clear:
            c.do_clear(&connection_store, diagnostic);
            break;
        case a_disconnect:
            c.do_disconnect(&connection_store, diagnostic);
            break;
        ...
        case a_x_call_request:
            c.do_x_call_request(&connection_store, msg_const_service(msg), msg_const_packet(msg), msg_const_window(msg), msg_const_throughput(msg));
            break;
