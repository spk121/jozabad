...
enum _action_t {
    ...
    a_x_call_accepted = 6,
    a_x_clear_request = 7,
    a_x_clear_confirmation = 8,
    a_x_data = 9,
    a_x_rr = 10,
    a_x_rnr = 11,
    a_x_reset = 12,
    a_x_reset_confirmation = 13,

    a_y_disconnect = 14,
    a_y_call_request = 15,
    a_y_call_accepted = 16,
    a_y_clear_request = 17,
    a_y_clear_confirmation = 18,
    a_y_data = 19,
    a_y_rr = 20,
    a_y_rnr = 21,
    a_y_reset = 22,
    a_y_reset_confirmation = 23,
...
enum _event_t {
    ...
    max_event = e_y_reset_confirmation,
    event_last = e_y_reset_confirmation
};
...
static void
    s_state_engine_do_error(parch_state_engine_t *self, diagnostic_t d);
static void
...
static int
s_state_engine_y_message(parch_state_engine_t *self, parch_msg_t *msg);
...
s_state_engine_do_error(parch_state_engine_t *self, diagnostic_t d) {
    ...
    parch_msg_set_cause(msg, remote_procedure_error);
    parch_msg_set_diagnostic(msg, d);
    s_state_engine_log_send_to_node(self, msg, d);
    ...
    if (peer) {
        ...
        parch_msg_set_cause(msg, remote_procedure_error);
        parch_msg_set_diagnostic(msg, d);
        s_state_engine_log_send_to_peer(self, msg, d);
        s_state_engine_y_message(self, msg);
...
s_state_engine_do_service(parch_state_engine_t *self) {
    ...
    // the broker level.
    char *sname = strdup(parch_msg_service_name(self->request));
    parch_node_set_service_name(self->node, sname);
    parch_node_update_service_name(self->node, parch_node_get_service_name(self->node));
}
...
s_state_engine_do_x_disconnect(parch_state_engine_t *self) {
    ...
    if (peer) {
        ...
        s_state_engine_log_send_to_peer(self, msg, err_none);
        s_state_engine_y_message(self, msg);
    }
    ...
    if (parch_broker_get_verbose(self->broker)) {
        ...
        zclock_log("I: [%s] state engine -- disconnect",
                    self_addr);
        free(self_addr);
    }
...
static void
state_engine_do_x_call_request(parch_state_engine_t * const self) {
    STATE_ENGINE_VALIDITY_CHECKS(self);
    ...
    if (parch_node_call_request(self->node, parch_msg_service_requested(self->request))) {
        ...
        node_t *peer = parch_node_get_peer(self->node);
        assert(peer);
        parch_state_engine_t *engine = parch_node_get_state_engine(peer);
        assert(engine);

        if (parch_broker_get_verbose(self->broker)) {
            char *self_addr = zframe_strhex(parch_node_get_node_address(self->node));
            char *peer_addr = zframe_strhex(parch_node_get_node_address(peer));
            zclock_log("I: [%s] state engine -- sending '%s' to peer [%s]",
                    self_addr,
                    parch_msg_command(msg),
                    peer_addr);
            free(self_addr);
        ...
        s_state_engine_y_message(engine, msg);
    } else {
        // If an idle node is not found, a clear request is sent to X.
        if (parch_broker_get_verbose(self->broker)) {
            char *self_addr = zframe_strhex(parch_node_get_node_address(self->node));
            zclock_log("I: [%s] state engine -- no peer available",
                    self_addr);
            free(self_addr);
        }
...
static void
state_engine_do_x_call_accepted(parch_state_engine_t *self) {
    STATE_ENGINE_VALIDITY_CHECKS(self);
    ...
    if (parch_broker_get_verbose(self->broker)) {
        ...
        zclock_log("I: [%s] state engine -- sending '%s' to peer [%s]",
                self_addr,
                parch_msg_command(reply),
                peer_addr);
        free(self_addr);
        free(peer_addr);
    }
    s_state_engine_y_message(engine, reply);
...
static void
state_engine_do_x_rr(parch_state_engine_t *self) {
    STATE_ENGINE_VALIDITY_CHECKS(self);
...
static void
state_engine_do_x_rnr(parch_state_engine_t *self) {
    STATE_ENGINE_VALIDITY_CHECKS(self);
...
state_machine_dispatch(parch_state_engine_t *self, action_t action, diagnostic_t diagnostic, state_t next) {
    ...
    switch (action) {
        ...
        case a_x_call_request:
            state_engine_do_x_call_request(self);
            break;
        case a_x_call_accepted:
            state_engine_do_x_call_accepted(self);
            break;
        ...
        case a_x_data:
            ...
            break;
        case a_x_rr:
            state_engine_do_x_rr(self);
            break;
        case a_x_rnr:
            state_engine_do_x_rnr(self);
            break;
        case a_x_reset:
