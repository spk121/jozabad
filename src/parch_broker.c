...
static service_t *
    s_broker_require_service(broker_t *self, char *service_name);
static void
    s_node_destroy(node_t **self_p);
static void
    s_node_freefn(void *vself);
static void
...
// This destroys the main broker object, and thus
// everything else.
static void
s_broker_destroy(broker_t **self_p) {
    assert(self_p);
...
// In response to a CONNECT_REQUEST message from an apparently
// new node, register it by name and attach it to a state machine
// engine.
static void
s_broker_register_node(broker_t *self, parch_msg_t *msg) {
    ...
    if (sname == 0 || strlen(sname) == 0 || !is_safe_ascii(sname)) {
        ...
        zhash_freefn(self->nodes, key, s_node_freefn);
        assert(rc != -1);
...
s_socket_event(zloop_t *loop __attribute__((unused)), zmq_pollitem_t *item, void *arg) {
    ...
    char *identity = zframe_strhex(parch_msg_address(msg));
    node_t *node_val = NULL;
    ...
    if (zhash_lookup(self->nodes, identity) == NULL) {
        if (parch_msg_id(msg) == PARCH_MSG_CONNECT)
            ...
            if (self->verbose) {
                char * msg_addr = zframe_strhex(parch_msg_address(msg));
    ...
    node_val = (node_t *) zhash_lookup(self->nodes, identity);

    if (parch_state_engine_x_message(node_val->engine, msg)) {
        parch_state_engine_destroy(&node_val->engine);
        zhash_delete(self->nodes, identity);
        s_node_destroy(&node_val);
    }
cleanup:
