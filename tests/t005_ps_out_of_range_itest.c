...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < 2; i ++) {
        ...
        // Send a data message
        if (verbose)
            printf("%s: sending data to broker with invalid PS %d\n", prefix, SEQ_MAX + 1);
        ret = joza_msg_send_data(sock, 0, 0, SEQ_MAX + 1, zframe_new(data, 4));
        if (ret != 0) {
            if (verbose)
        ...
        if (joza_msg_id(response) != JOZA_MSG_DIAGNOSTIC) {
            ...
            exit (1);
        } else if(joza_msg_cause(response) != c_malformed_message) {
            if (verbose)
            ...
            exit(1);
        } else if (joza_msg_diagnostic(response) != d_ps_too_large) {
            if (verbose)
