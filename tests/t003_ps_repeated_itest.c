...
int main(int argc, char** argv)
{
    ...
    initialize (verbose, "peer X", &ctx1, &sock1, broker, calling_address1, "Adam Smith", dir);
    initialize (verbose, "peer Y", &ctx2, &sock2, broker, calling_address2, "Eve Plum", dir);
    ...
    for (int i = 0; i < 2; i ++) {
        ...
        if (verbose)
            printf("%s: sending data to broker with repeated PS = 0\n", prefix);
        ret = joza_msg_send_data(sock, 0, 0, 0, zframe_new(data, 4));
        if (ret != 0) {
            if (verbose)
                printf("%s: joza_msg_send_data (...) returned %d\n", prefix, ret);
