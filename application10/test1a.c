...
int main(int argc, char **argv)
{
  ...
  client_send_then_free_msg (connection_X,
                             jz_msg_new_call_request(LCN_X, "ADAM", "EVE", p_default, WINDOW_MAX, t_default, NULL, 0));
  GET(in_Y);

  client_send_then_free_msg (connection_Y,
                             jz_msg_new_call_accepted(1, "EVE", "ADAM", p_default, WINDOW_MAX, t_default, NULL, 0));
  GET(in_X);

  
  client_send_then_free_msg (connection_X,
                             jz_msg_new_data(LCN_X, 0, 0, 0, g_strdup ("MESSAGE1"), 8));
  GET(in_Y);
  client_send_then_free_msg (connection_X,
                             jz_msg_new_data(LCN_X, 0, 0, 1, g_strdup ("MESSAGE2"), 8));
  GET(in_Y);
  client_send_then_free_msg (connection_X,
                             jz_msg_new_data(LCN_X, 0, 0, 2, g_strdup ("MESSAGE3"), 8));
  GET(in_Y);

  client_send_then_free_msg (connection_Y,
							jz_msg_new_reset_request (LCN_Y, c_reset__client_originated, 0));
  GET(in_X);

  client_send_then_free_msg (connection_X,
							 jz_msg_new_reset_confirmation (LCN_X));
  GET(in_Y);
